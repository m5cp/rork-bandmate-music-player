...
struct PracticeReportView: View {
    ...
    private func buildShareItems() -> [Any] {
        let cardImage = renderShareableCard(
            score: feedback.overallScore,
            instrument: instrument,
            songTitle: feedback.summary.prefix(60).description,
            streak: GamificationManager.shared.currentStreak,
            focusAreas: feedback.focusAreas
        )

        let text = "I scored \(feedback.overallScore)/100 practicing \(instrument.rawValue) on KITB! 🎵"
        return [cardImage, text]
    }
}
